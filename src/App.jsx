...
export default function App() {
  ...
  useEffect(() => {
    ...
    twemoji.parse(appRootRef.current, { folder: "svg", ext: ".svg" });
  }, [data, selectedSectionId, selectedCategoryId, accessMode, darkMode, showGallery, showCollabModal, showLoginModal, editMode, isAuthenticated]);

  return (
    <div ref={appRootRef} style={{ background: theme.bg, color: theme.text, height: "100vh", display: "flex", fontFamily: "'Inter', -apple-system, 'Segoe UI Emoji', 'Apple Color Emoji', 'Noto Color Emoji', 'Twemoji Mozilla', sans-serif", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 32 }}>
          <div style={{ maxWidth: 760, width: "100%", padding: 32, borderRadius: 16, backgroundColor: theme.panel, border: `1px solid ${theme.border}`, boxShadow: theme.shadow, display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
              ...
              <button onClick={() => setDarkMode((d) => !d)} style={{ padding: "10px 16px", borderRadius: 12, backgroundColor: theme.panel, color: theme.text, border: `1px solid ${theme.border}`, cursor: "pointer" }}>{darkMode ? "☀️" : "🌙"}</button>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 16 }}>
              <button onClick={handleSelectVisitor} style={{ padding: "16px", borderRadius: 12, border: `1px solid ${theme.border}`, background: `linear-gradient(135deg, ${theme.panel} 0%, ${theme.input} 100%)`, color: theme.text, cursor: "pointer", fontSize: 18, fontWeight: 600, boxShadow: theme.shadow }}>👀 Mode Visiteur</button>
              <button onClick={handleOpenAdminLogin} style={{ padding: "16px", borderRadius: 12, border: `1px solid ${theme.border}`, background: `linear-gradient(135deg, ${theme.accent1} 0%, ${theme.accent2} 100%)`, color: "white", cursor: "pointer", fontSize: 18, fontWeight: 700, boxShadow: theme.shadow }}>🛠️ Mode Admin</button>
            </div>
          </div>
        </div>
      )}
